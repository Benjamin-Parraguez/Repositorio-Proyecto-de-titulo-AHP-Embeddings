...
archivo_excel = r"C:\Users\rikig\OneDrive\Escritorio\Proyecto de Título\Mesa 5\Metodos Multicriterios\AHP\Python\AHP_CasoExtremo2.5.xlsx"  # <- Cambia por tu nombre de archivo
...
# Leer matriz de criterios
matriz_criterios_df = pd.read_excel(xls, sheet_name="criterios", index_col=0)
criterios = list(matriz_criterios_df.columns)
matriz_criterios = matriz_criterios_df.applymap(fraccion_a_float).to_numpy()

# Leer matrices de alternativas para cada criterio
...
# Calcular pesos locales de alternativas (sin cambios)
pesos_locales = [calcular_pesos(matrices_alternativas[c]) for c in criterios]


pesos_globales = sum(pesos_locales[i] * pesos_criterios[i] for i in range(len(criterios)))
lambda_max, CI, CR = consistencia_ahp(matriz_criterios)
...
df_consistencia = pd.DataFrame({
    ...
    'CI': [CI],
    'CR': [CR],
    'Consistencia aceptable': ['Sí' if CR < 0.1 else 'No']
})

# === Paso 5: Exportar a Excel con formato ===
archivo_salida = r"C:\Users\rikig\OneDrive\Escritorio\Proyecto de Título\Mesa 5\Metodos Multicriterios\AHP\Python\R_AHP_CasoExtremo2.5.xlsx"
...
with pd.ExcelWriter(archivo_salida, engine='openpyxl', mode='a') as writer:
    # 1. Exportar matriz de criterios original
    matriz_criterios_df.to_excel(writer, sheet_name='Matriz_Criterios')
...
for r in range(1, len(nombre_criterios) + 1):
    for combo in itertools.combinations(nombre_criterios, r):
        ...
        etiquetas.append("-".join([abreviaturas[c] for c in combo]))

# === Asegurar formato de matriz local ===
matriz_local = np.array(pesos_locales)  # Tamaño: (criterios, alternativas)

# === Recalcular pesos globales para cada combinación ===
...
for idx, combo in enumerate(combinaciones):
    ...
    for i, criterio in enumerate(nombre_criterios):
        if criterio in combo:
            pesos_combo[i] = 1 / len(combo)  # Distribución equitativa
